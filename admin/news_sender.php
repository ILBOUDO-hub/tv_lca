...
if(isset($_POST['submit'])){
    ...
    $date_news = htmlspecialchars($_POST['dates_news']) ;
    $titre = htmlspecialchars($_POST['titre']) ;
    $type_news = htmlspecialchars($_POST['types_news']) ;
    $contenu = htmlspecialchars($_POST['contenu']) ;
    
    $photo_name = $_FILES['photo']['name'];
    $target= 'upload/news/'.basename($photo_name);
    $info= pathinfo($_FILES['photo']['name']);
    $path = $info['extension'] ;
    if(in_array($path,array("jpg","png","jpeg","JPG","JPEG","PNG","jfif"))){
        if($_FILES['photo']['size']<10000000000){
            move_uploaded_file($_FILES['photo']['tmp_name'], 'upload/news/' . basename($photo_name));
            $bdd= new PDO("mysql:host=localhost;dbname=tv_lca","root","");
            $bdd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $query =$bdd->prepare("INSERT INTO `tv_lca`.`news` (`id_news`, `auteur`,`dates_news`, `titre`,`photo`, `types_news`, `contenu`) VALUES (NULL,?,?,?,?,?,?)");
            $query->execute(array($auteur,$date_news,$titre,$target,$type_news,$contenu));
...
<body>
				...
				<li class="text-center">
                    ...
                    <li>
                        <a href="replay_sender.php"><i class="fa fa-check-circle fa-3x"></i> Emissions</a>
					          ...
					          <li>
                        <a class="active-menu" href="news_sender.php"><i class="fa fa-bar-chart-o fa-3x"></i> News</a>
                    </li>	
                </ul>
    ...
    <div class="container">
        <h2 style="text-align: center;">Poster une news</h2>
        <div class="row">
          <section class="col-xs-10">
      
      <form class="form-horizontal" method="POST" enctype="multipart/form-data" method="news_sender.php">
      <div class="form-group">
        <label class="col-sm-2 control-label"
          for="auteur">Auteur</label>
        <div class="col-sm-10">
        <input class="form-control" type="text"
          name="auteur" placeholder="Auteur de l'article" required>      
        </div>
      </div>

      <div class="form-group">
        <label class="col-sm-2 control-label"
          for="dates_news">Date</label>
        <div class="col-sm-10">
        <input class="form-control" type="date"
          name="dates_news" placeholder="Date" required>
        </div>
      </div>
      ...
      <div class="form-group">
        <label class="col-sm-2 control-label"
          for="types_news">Type d'événement</label>
        <div class="col-sm-10">
        <select class="form-control" name="types_news" id="types_news" required>
...
<?php
    ...
    $bdd= new PDO("mysql:host=localhost;dbname=tv_lca","root","");
    $bdd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $resultat = $bdd->query("SELECT id_news, titre, auteur, photo FROM `news`  ORDER BY `id_news` desc");
	if($resultat){
		...
		echo "<td><strong>Auteur</strong></td>\n";
