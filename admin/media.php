    ...
    <div class="container">
      ...
      <div class="form-group">
        ...
        <input class="form-control" type="text"
          name="title" placeholder="Entrer le lien de la vidéo" required>
        </div>
      </div>

      <div class="form-group">
        <label class="col-sm-2 control-label"
          for="dates_news">Titre</label>
        <div class="col-sm-10">
        <input class="form-control" type="text"
          name="titre" placeholder="Entrer le titre de l'émission" required>
        </div>
      </div>
...
<?php
    ...
    $bdd= new PDO("mysql:host=localhost;dbname=tv_lca","root","");
    $bdd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $resultat = $bdd->query("SELECT * FROM `media`  ORDER BY `id_media` desc");
	if($resultat){
