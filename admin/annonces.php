...
if(isset($_POST['post'])){
    ...
    $target= 'upload/'.basename($photo_name);
    $info= pathinfo($_FILES['photo']['name']);
    $path = $info['extension'] ;
    if(in_array($path,array("jpg","png","jpeg","JPG","JPEG","PNG","jfif"))){
        if($_FILES['photo']['size']<10000000000){
            move_uploaded_file($_FILES['photo']['tmp_name'], 'upload/' . basename($photo_name));
            $bdd= new PDO("mysql:host=localhost;dbname=tv_lca","root","");
            $bdd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $query =$bdd->prepare("INSERT INTO `tv_lca`.`annonces` (`id_annonces`, `photo`, `commentaires`) VALUES (NULL, ?, ?)");
            $query->execute(array($target,$commentaires));
        }else{
            echo"<h1>too big</h1>";
        }
    }else{
...
<body>
    ...
    <a href="users.php" class="btn btn-danger square-btn-adjust">Logout</a></div>
      ...
      <div class="container">
        <h2 style="text-align: center;">Poster un média</h2>
        <div class="row">
      ...
      <div class="form-group">
        <label class="col-sm-2 control-label"
          for="contenu">Commentaire</label>
        <div class="col-sm-10">
        <textarea class="form-control" 
          name="commentaires" required></textarea>
        </div>
      </div>
      
      <div class="form-group">
      <div class="col-sm-10 col-sm-offset-2">
        <input type="submit" class="btn btn-default" name="post" value="submit">
      </div>
      </div>  
      </form>
      </section>
    ...
    $bdd= new PDO("mysql:host=localhost;dbname=tv_lca","root","");
    $bdd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $resultat = $bdd->query("SELECT * FROM `annonces`  ORDER BY `id_annonces` desc");
	if($resultat){
		echo "<h1 style=\"text-align:center;color: red\";>Liste des medias</h1>\n";
